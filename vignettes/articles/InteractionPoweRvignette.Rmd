...
We can also use the function `power_interaction` to compute power via monte-carlo simulation. Additional parameters include `n.iter`, which is the number of simulations run. In this example we'll use just 1,000 simulations, but we recommend using 10,000 for more stable results. Also note that in all these examples the parameter `seed` is set. This is the seed for the random number generator, which will allow you to obtain the same results. However, we recommend not setting `seed` when running your own power analyses, at least until you are ready to share the results. Each time an analysis is run a new seed will be chosen and echoed, so that you will be able to reproduce your results.
...
power_test = power_interaction(
                  ...
                  adjust.correlations = FALSE,cl=2)     # Adjust correlations for skew?  
